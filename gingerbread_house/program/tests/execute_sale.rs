...
async fn execute_sale_early_failure() {
    ...
    let accounts = mpl_gingerbread_house::accounts::GingerbreadHouseExecuteSale {
...
async fn execute_sale_success() {
    ...
    let accounts = mpl_gingerbread_house::accounts::GingerbreadHouseExecuteSale {
        ...
        buyer: buyer.pubkey(),
        seller: test_metadata.token.pubkey(),
        listing_auction_house: ahkey,
        bidding_auction_house: ahkey,
        metadata: test_metadata.pubkey,
        token_account: sell_acc.token_account,
        authority: ah.authority,
        seller_trade_state: sell_acc.seller_trade_state,
        ...
        token_mint: test_metadata.mint.pubkey(),
        auction_house_fee_account: ah.auction_house_fee_account,
        listing_auction_house_treasury: ah.auction_house_treasury,
        bidding_auction_house_treasury: ah.auction_house_treasury,
        treasury_mint: ah.treasury_mint,
        program_as_signer: sell_acc.program_as_signer,
        system_program: system_program::id(),
        ata_program: spl_associated_token_account::id(),
...
#[tokio::test]
async fn execute_open_sale_success() {
    let mut context = auctioneer_program_test().start_with_context().await;
    // Payer Wallet
    let (listing_ah, listing_ahkey, listing_authority) =
        existing_auction_house_test_context(&mut context)
            .await
            .unwrap();
    let (bidding_ah, bidding_ahkey, bidding_authority) =
        existing_auction_house_test_context(&mut context)
            .await
            .unwrap();
    let listing_test_metadata = Metadata::new();
    let mut bidding_test_metadata = Metadata::new();
    bidding_test_metadata.mint =
        Keypair::from_bytes(&listing_test_metadata.mint.to_bytes()).unwrap();
    bidding_test_metadata.token =
        Keypair::from_bytes(&listing_test_metadata.token.to_bytes()).unwrap();
    airdrop(
        &mut context,
        &listing_test_metadata.token.pubkey(),
        10_000_000_000,
    )
    .await
    .unwrap();
    airdrop(
        &mut context,
        &bidding_test_metadata.token.pubkey(),
        10_000_000_000,
    )
    .await
    .unwrap();
    listing_test_metadata
        .create(
            &mut context,
            "Test".to_string(),
            "TST".to_string(),
    ...
    context
        ...
        .unwrap();
    let buyer_token_account =
        get_associated_token_address(&buyer.pubkey(), &bidding_test_metadata.mint.pubkey());

    context.warp_to_slot(120 * 400).unwrap();
    context.warp_to_slot(120 * 400).unwrap();

    let (auctioneer_pda, _) = find_auctioneer_pda(&bidding_ahkey, &mpl_gingerbread_house::id());
    let accounts = mpl_gingerbread_house::accounts::GingerbreadHouseExecuteSale {
        auction_house_program: mpl_auction_house::id(),
        listing_config: listing_config_address,
        buyer: buyer.pubkey(),
        seller: listing_test_metadata.token.pubkey(),
        listing_auction_house: listing_ahkey,
        bidding_auction_house: bidding_ahkey,
        metadata: listing_test_metadata.pubkey,
        token_account: sell_acc.token_account,
        authority: bidding_ah.authority,
        seller_trade_state: sell_acc.seller_trade_state,
        buyer_trade_state: bid_acc.buyer_trade_state,
        token_program: spl_token::id(),
        free_trade_state: sell_acc.free_seller_trade_state,
        seller_payment_receipt_account: listing_test_metadata.token.pubkey(),
        buyer_receipt_token_account: buyer_token_account,
        escrow_payment_account: bid_acc.escrow_payment_account,
        token_mint: listing_test_metadata.mint.pubkey(),
        auction_house_fee_account: bidding_ah.auction_house_fee_account,
        listing_auction_house_treasury: listing_ah.auction_house_treasury,
        bidding_auction_house_treasury: bidding_ah.auction_house_treasury,
        treasury_mint: listing_ah.treasury_mint,
        program_as_signer: sell_acc.program_as_signer,
        system_program: system_program::id(),
        ata_program: spl_associated_token_account::id(),
        rent: sysvar::rent::id(),
    ...
    let (_, free_sts_bump) = find_trade_state_address(
        ...
        &listing_ahkey,
        &sell_acc.token_account,
        &listing_ah.treasury_mint,
        &listing_test_metadata.mint.pubkey(),
        0,
        1,
    );
    let (_, escrow_bump) = find_escrow_payment_address(&bidding_ahkey, &buyer.pubkey());
    let (_, pas_bump) = find_program_as_signer_address();
    ...
    airdrop(
        &mut context,
        &bidding_ah.auction_house_fee_account,
        10_000_000_000,
    )
    .await
    .unwrap();
    ...
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&bidding_authority.pubkey()),
        &[&bidding_authority],
        context.last_blockhash,
    );
    let seller_before = context
        .banks_client
        .get_account(listing_test_metadata.token.pubkey())
        .await
        .unwrap()
        .unwrap();
    let buyer_token_before = &context
    ...
    .unwrap();
    let fee_minus: u64 =
        100_000_000 - ((listing_ah.seller_fee_basis_points as u64 * 100_000_000) / 10000);
    assert_eq!(seller_before.lamports + fee_minus, seller_after.lamports);
    assert_eq!(seller_before.lamports < seller_after.lamports, true);
    assert_eq!(buyer_token_after.amount, 1);
}
