...
pub struct GingerbreadHouseExecuteSale<'info> {
    ...
    #[account(
        seeds=[
            ...
            listing_auction_house.treasury_mint.as_ref(),
    ...
    #[account(mut, seeds=[PREFIX.as_bytes(), bidding_auction_house.key().as_ref(), buyer.key().as_ref()], seeds::program=auction_house_program, bump=escrow_payment_bump)]
    ...
    #[account(seeds=[PREFIX.as_bytes(), listing_auction_house.creator.as_ref(), listing_auction_house.treasury_mint.as_ref()], seeds::program=auction_house_program, bump=listing_auction_house.bump/*, has_one=authority*/, has_one=treasury_mint/*, has_one=auction_house_fee_account*/)]
    pub listing_auction_house: Box<Account<'info, AuctionHouse>>,

    /// Bidding Auction House instance PDA account.
    #[account(seeds=[PREFIX.as_bytes(), bidding_auction_house.creator.as_ref(), bidding_auction_house.treasury_mint.as_ref()], seeds::program=auction_house_program, bump=bidding_auction_house.bump/*, has_one=authority*/, has_one=treasury_mint/*, has_one=auction_house_fee_account*/)]
    pub bidding_auction_house: Box<Account<'info, AuctionHouse>>,

    /// CHECK: Not dangerous. Account seeds checked in constraint.
    /// Auction House instance fee account.
    #[account(mut, seeds=[PREFIX.as_bytes(), bidding_auction_house.key().as_ref(), FEE_PAYER.as_bytes()], seeds::program=auction_house_program, bump=bidding_auction_house.fee_payer_bump)]
    pub auction_house_fee_account: UncheckedAccount<'info>,

    /// CHECK: Not dangerous. Account seeds checked in constraint.
    /// Listing Auction House instance treasury account.
    #[account(mut, seeds=[PREFIX.as_bytes(), listing_auction_house.key().as_ref(), TREASURY.as_bytes()], seeds::program=auction_house_program, bump=listing_auction_house.treasury_bump)]
    pub listing_auction_house_treasury: UncheckedAccount<'info>,

    /// CHECK: Not dangerous. Account seeds checked in constraint.
    /// Bidding Auction House instance treasury account.
    #[account(mut, seeds=[PREFIX.as_bytes(), bidding_auction_house.key().as_ref(), TREASURY.as_bytes()], seeds::program=auction_house_program, bump=bidding_auction_house.treasury_bump)]
    ...
    #[account(mut, seeds=[PREFIX.as_bytes(), seller.key().as_ref(), listing_auction_house.key().as_ref(), token_account.key().as_ref(), listing_auction_house.treasury_mint.as_ref(), token_mint.key().as_ref(), &u64::MAX.to_le_bytes(), &token_size.to_le_bytes()], seeds::program=auction_house_program, bump=seller_trade_state.to_account_info().data.borrow()[0])]
    pub seller_trade_state: UncheckedAccount<'info>,

    /// CHECK: Not dangerous. Account seeds checked in constraint.
    /// Free seller trade state PDA account encoding a free sell order.
    #[account(mut, seeds=[PREFIX.as_bytes(), seller.key().as_ref(), listing_auction_house.key().as_ref(), token_account.key().as_ref(), listing_auction_house.treasury_mint.as_ref(), token_mint.key().as_ref(), &0u64.to_le_bytes(), &token_size.to_le_bytes()], seeds::program=auction_house_program, bump=free_trade_state_bump)]
    ...
    #[account(seeds = [AUCTIONEER.as_bytes(), bidding_auction_house.key().as_ref(), auctioneer_authority.key().as_ref()], seeds::program=auction_house_program, bump = bidding_auction_house.auctioneer_pda_bump)]
...
pub fn gingerbread_house_execute_sale<'info>(
    ...
    let cpi_accounts = AHExecuteSale {
        ...
        listing_auction_house: ctx.accounts.listing_auction_house.to_account_info(),
        bidding_auction_house: ctx.accounts.bidding_auction_house.to_account_info(),
        auction_house_fee_account: ctx.accounts.auction_house_fee_account.to_account_info(),
        listing_auction_house_treasury: ctx
            .accounts
            .listing_auction_house_treasury
            .to_account_info(),
        bidding_auction_house_treasury: ctx
            .accounts
            .bidding_auction_house_treasury
            .to_account_info(),
